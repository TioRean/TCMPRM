...
output_template = {
    ...
    # Based on the given traditional Chinese medicine case records and prescription details, analyze the case's principles, methods, formulas, and herbs in concise language. Review and summarize relevant knowledge, and finally write a prescription with dosages, decoction and administration methods, and precautions.
    },
    'requirements': {
        ...
        # Express in concise language.
        # Write in the style and format of a modern TCM practitioner.
        'diagnosis_prompts': ['除非提及中西医诊断，否则不书写任何诊断结果'],
        # Unless Traditional Chinese Medicine or Western Medicine diagnosis is mentioned, do not write any diagnostic results.
        'mdf_exp': [
            '分析方药时，若结构化信息提供了加减法注释，将加减法注释改写精简且尽可能包含原信息，体现药证，可适当根据加减法注释推理加减法部分的中医病机'],
        # When analyzing formulas and herbs, if structured information provides modification notes (additions/subtractions), rewrite and simplify these notes to be concise while preserving the original information as much as possible, reflecting the correspondence between herbs and symptoms. It is appropriate to infer the underlying traditional Chinese medicine (TCM) pathogenesis of the modification part based on the modification notes.
        'prescriptions': ['书写处方时不加减方药，不改变原方药、不改变方药剂量、不改变方药剂量单位、不改变煎服法'],
        # When writing a prescription, do not add or subtract herbs, do not alter the original formula, do not change the dosage of the herbs, do not modify the dosage units, and do not change the decoction and administration methods.
        'notes': ['可适当补充必要的注意事项'],
        # You may appropriately supplement with necessary precautions.
        'explanation_prompts': ['禁止添加关于本病例文本如何被编写的“元说明”和自我评价',
                                '生成内容中禁止提到任何上述文本生成要求的内容']
        # It is prohibited to add any 'meta-explanations' or self-evaluations regarding how this case text was written.
        # The generated content must not mention any of the above text generation requirements.
    }
}
