...
input_template_1 = {
    ...
    'task': {
        ...
        # Please translate the following structured information into a natural-language written Traditional Chinese Medicine medical case.
    ...
    'requirements': {
        ...
        # After synonym substitution of structured information, rewrite it into natural language.
        # Generated content shall only pertain to medical history information; it is prohibited to generate content related to TCM etiology and pathogenesis, syndrome differentiation analysis, four diagnostic methods analysis, differentiation results, treatment plans, prescriptions, formula composition, decoction methods, or precautions.
        # Do not alter the original meaning of the information.
        # Avoid unnecessary repetition in wording.
        # Use language that conforms to medical standards and is logically clear.
        # Ensure the content and all time points are logical, realistic, and avoid self-contradiction within the text.
        'style_prompts': [
            '将结构化信息改写为中医医案，医案按照主诉、现病史、刻下、查体、既往史、个人史、辅助检查的标准顺序格式书写',
            '女性患者需额外写月经史、婚育史',
            '不必保留原结构化信息的格式'],
        # Translate the structured information into a traditional Chinese medicine medical case record. The case should be written in the standard sequence: chief complaint, present illness, current symptoms, physical examination, past medical history, personal history, and auxiliary examination.
        # For female patients, additionally include menstrual history and marital/obstetric history.
        # It is not necessary to retain the original structured format.
        ...
        # The chief complaint should not exceed 20 characters.
        # If no chief complaint is provided, select the most urgent symptom or sign from the clinical manifestations as the chief complaint; it does not necessarily have to be the first few listed.
        # If the clinical manifestations involve multisystem diseases, choose a disease from a single system as the primary diagnosis, which should correspond to the chief complaint.
        # Although TCM often involves symptoms and signs across multiple systems, the chief complaint must not correspond to multiple unrelated systems or Western medicine diagnoses. Instead, it should correspond to a single system or a single Western medicine diagnosis; other information need not be reflected in the chief complaint.

        'sym_phy_prompts': [
            '结构化信息中临床表现为乱序，需以主诉为核心调整书写顺序，主次分明，相关的临床表现相邻',
            '参考病史信息和对应处方添加指定的额外信息，但添加的额外信息不影响处方结果',
            # In structured information, the clinical manifestations are presented out of order; the writing sequence should be adjusted with the chief complaint as the core, ensuring clear prioritization and placing related clinical manifestations adjacently.
            # Add specified additional information by referring to the medical history and corresponding prescriptions, but ensure that the added information does not affect the prescription outcome.
            ['适当添加与当前辨证论治无关的症状体征信息，不得额外增加支持当前辨证论治结果的其他症状体征',
             ''],
            # Appropriately add information on symptoms and signs unrelated to the current syndrome differentiation and treatment, but do not additionally include other symptoms or signs that support the current syndrome differentiation and treatment outcome.
            '按病例书写习惯在刻下症一栏中添加适当的阴性信息',
            '若未指定一般情况、进食、睡眠、大便、小便、查体，需按病例书写习惯补充相关阴性信息',
            '若个别临床表现与整体明显不符，可删去',
            '若舌象或脉象信息存在矛盾，需考虑不同部位存在不同舌象脉象的可能'],
            # Add appropriate negative information in the 'Current Symptoms' section according to medical record writing conventions.
            # If general condition, diet, sleep, bowel movements, urination, or physical examination are not specified, relevant negative information should be supplemented as per medical record writing practices.
            # If individual clinical manifestations are significantly inconsistent with the overall presentation, they may be deleted.
            # If there are contradictions in tongue or pulse signs, the possibility of different tongue and pulse manifestations in different body regions should be considered.

        'pre_treat_prompts': ['可按病例书写习惯适当添加既往治疗的中西药物', ],
        # According to the standard practice of medical record writing, it is appropriate to add details about previous treatments involving both traditional Chinese and Western medications as needed.
        'aux_exam_prompts': ['可按病例书写习惯生成额外的支持诊断或鉴别诊断的辅助检查内容', ],
        # It can generate additional auxiliary examination content to support diagnosis or differential diagnosis according to medical record writing conventions.
        'past_med_his_prompts': [
            '若结构化信息中未指定既往史、个人史、婚育史、月经史，需按病例书写习惯补充相关阴性信息',
            '如有必要，可按病例书写习惯适当添加有利于诊断或鉴别诊断的既往史、个人史、婚育史、月经史',
            # If the structured information does not specify past medical history, personal history, marital and reproductive history, or menstrual history, relevant negative findings should be added according to standard medical record documentation practices.
            # When necessary, additional past medical history, personal history, marital and reproductive history, or menstrual history that may aid in diagnosis or differential diagnosis can be appropriately added following standard medical record documentation practices.

            [{'prompt': '患者既往有<space_0>相关基础病，<space_1>手术病史，随机为其添加相关既往病史',
              # The patient has a history of <space_0>-related underlying diseases and a surgical history of <space_1>. Relevant past medical history is randomly added for them.
              'space': [
                  [['循环系统', '呼吸系统', '消化系统', '神经系统', '运动系统',
                    '泌尿生殖系统'], (1, 2)],
                  #  Circulatory system, Respiratory system, Digestive system, Nervous system, Locomotor system, Urogenital system
                  [['无', '1种', '2种'], 1],
              ]}, '', '', '', ]],
        'diagnosis_prompts': ['若未提供诊断信息，则不书写诊断结果部分',
                              ],
        # If no diagnostic information is provided, the diagnosis section should not be written.
        'explanation_prompts': ['禁止添加关于本病例文本如何被编写的“元说明”和自我评价',
                                '生成内容中禁止提到任何上述文本生成要求的内容']
        # Do not add any 'meta-commentary' or self-evaluation regarding how this case text was written.
        # Generated content must not mention any of the above text generation requirements.
    }
...
input_templates_2 = {
    ...
    'requirements': {
        ...
        # Write in the format and linguistic style of modern and contemporary traditional Chinese medicine medical records, without necessarily retaining the original structured format, and without following the sequence of chief complaint, present illness, past history, etc.
        'chief_complaint_prompts': ['确保主诉精简规范，主诉症状或体征合计不超过3个',
                                    '主诉不超过20个字',
                                    '若未给定主诉，则从临床表现中选择最急需解决的症状或体征作为主诉，主诉未必是前几个临床表现',
                                    '若临床表现涉及多系统疾病，选择单个系统的疾病作为主要诊断，主要诊断与主诉相对应',
        ...
        # Ensure the chief complaint is concise and standardized, with no more than 3 symptoms or signs combined.
        # The chief complaint should not exceed 20 characters.
        # If no chief complaint is given, select the most urgent symptom or sign from the clinical manifestations as the chief complaint; it need not be the first few listed.
        # If the clinical manifestations involve multiple system diseases, choose a single system's disease as the primary diagnosis, which should correspond to the chief complaint.
        # Although TCM information often involves symptoms and signs across multiple systems, the chief complaint must not correspond to multiple unrelated systems or Western medical diagnoses; it should correspond to a single system or a single Western medical diagnosis, with other information omitted from the chief complaint.
        'sym_phy_prompts': [
            '结构化信息中临床表现为乱序，需以主诉为核心调整书写顺序，主次分明，相关的临床表现相邻'
            '参考病史信息和对应处方添加指定的额外信息，但添加的额外信息不影响处方结果',
            # In structured information, the clinical manifestations are presented out of order; they need to be reorganized with the chief complaint as the core, ensuring clear prioritization and placing related clinical manifestations adjacently.
            # Refer to the medical history and corresponding prescriptions to add specified additional information, but the added information should not affect the prescription outcome.
            [
                '适当添加与当前辨证论治无关的症状体征信息，但不影响最终辨证结果，不得额外增加支持当前辨证论治结果的其他症状体征',
                ''],
            # Appropriately add information about symptoms and signs that are unrelated to the current syndrome differentiation and treatment, but do not affect the final differentiation result. Do not additionally include other symptoms and signs that would support the current syndrome differentiation and treatment outcome.
            '按病例书写习惯在刻下症一栏中添加适当的阴性信息',
            '若未指定一般情况、进食、睡眠、大便、小便、查体，需按病例书写习惯补充相关阴性信息',
            '若个别临床表现与整体明显不符，可删去',
            '若舌象或脉象信息存在矛盾，需考虑不同部位存在不同舌象脉象的可能'],
            # Add appropriate negative findings in the column for current symptoms according to standard medical record writing conventions.
            # If general condition, diet, sleep, bowel movements, urination, or physical examination findings are not specified, supplement relevant negative information as per medical record writing norms.
            # Remove individual clinical manifestations if they significantly contradict the overall presentation.
            # If there are contradictions in tongue or pulse signs, consider the possibility of different manifestations in different body regions.
        'pre_treat_prompts': ['可适当添加既往治疗的中西药物', ],
        # You may appropriately add previously used Chinese and Western medications.
        'aux_exam_prompts': ['可生成额外的支持诊断或鉴别诊断的辅助检查内容', ],
        # It can generate additional auxiliary examination content to support diagnosis or differential diagnosis.
        'past_med_his_prompts': [
            '如有必要，可适当添加有利于诊断或鉴别诊断的既往史、个人史、婚育史、月经史',
            # If necessary, appropriate past medical history, personal history, marital and obstetric history, and menstrual history that may aid in diagnosis or differential diagnosis can be added.
            [{'prompt': '患者既往有<space_0>相关基础病，<space_1>手术病史，随机为其添加相关既往病史',
              # The patient has a previous history of <space_0>-related underlying diseases and a history of <space_1> surgery. Relevant past medical history will be randomly added for them.
              'space': [
                  [['循环系统', '呼吸系统', '消化系统', '神经系统', '运动系统',
                    '泌尿生殖系统'], (1, 2)],
                  #  Circulatory system, Respiratory system, Digestive system, Nervous system, Locomotor system, Urogenital system
        ...
        'diagnosis_prompts': ['若未提供诊断信息，则不书写诊断结果部分', ],
        # If no diagnostic information is provided, the diagnostic results section should not be written.
    }
...
input_templates_3 = {
    ...
    'requirements': {
        ...
        # Rewrite structured information into colloquial traditional Chinese medicine case records.
        # Do not retain the original structured format.
        'chief_complaint_prompts': [
            '若未给定主诉，则从临床表现中选择最急需解决的症状或体征作为主诉，主诉未必是前几个临床表现',
            '若临床表现涉及多系统疾病，选择单个系统的疾病作为主要诊断，主要诊断与主诉相对应',
            '虽然中医信息常涉及多个系统的症状体征，但是主诉书写时禁止对应多个不相关的系统或西医诊断，而应对应单个系统或单个西医诊断，其余信息无需在主诉中体现',
        ],
        # If no chief complaint is given, select the most urgent symptom or sign from the clinical manifestations as the chief complaint. The chief complaint does not necessarily have to be one of the first few clinical manifestations listed.
        # If clinical manifestations involve multiple systemic diseases, choose a disease from a single system as the primary diagnosis, and ensure the primary diagnosis corresponds to the chief complaint.
        # Although traditional Chinese medicine information often involves symptoms and signs from multiple systems, when writing the chief complaint, it is prohibited to correspond to multiple unrelated systems or Western medicine diagnoses. Instead, it should correspond to a single system or a single Western medicine diagnosis, with the remaining information not needing to be reflected in the chief complaint.
        'sym_phy_prompts': [
            '结构化信息中临床表现为乱序，需以主诉为核心调整书写顺序，主次分明，相关的临床表现相邻',
            '参考病史信息和对应处方添加指定的额外信息，但添加的额外信息不影响处方结果',
            # In structured information, clinical manifestations are presented out of order. The writing sequence should be adjusted with the chief complaint as the core, ensuring clarity between primary and secondary aspects, and keeping related clinical manifestations adjacent.
            # Refer to the medical history information and corresponding prescriptions to add specified additional information, but the added information should not affect the prescription outcome.
            [
                '适当添加与当前辨证论治无关的症状体征信息，但不影响最终辨证结果，不得额外增加支持当前辨证论治结果的其他症状体征',
                ''],
            # Appropriately add symptom and sign information unrelated to the current syndrome differentiation and treatment, but ensure it does not affect the final diagnosis outcome. Do not additionally include other symptoms or signs that would support the current syndrome differentiation and treatment result.
            '按病例书写习惯在刻下症处添加适当的阴性信息',
            '若未指定一般情况、进食、睡眠、大便、小便、查体，需按病例书写习惯补充相关阴性信息',
            '若个别临床表现与整体明显不符，可删去',
            '若舌象或脉象信息存在矛盾，需考虑不同部位存在不同舌象脉象的可能'],
        # "Add appropriate negative information in the 'current symptoms' section according to medical record writing conventions."
        # "If general condition, diet, sleep, bowel movements, urination, or physical examination are not specified, supplement relevant negative information according to medical record writing practices."
        # "If individual clinical manifestations are significantly inconsistent with the overall presentation, they may be omitted."
        # "If there are contradictions in tongue or pulse signs, consider the possibility of different tongue and pulse manifestations in different body regions."
        'pre_treat_prompts': ['可适当添加既往治疗的中西药物', ],
        # Appropriate addition of previously administered Chinese and Western medications may be considered.
        'aux_exam_prompts': ['可生成额外的支持诊断或鉴别诊断的辅助检查内容', ],
        # Can generate additional auxiliary examination content to support diagnosis or differential diagnosis.
        'past_med_his_prompts': [
            '如有必要，可适当添加有利于诊断或鉴别诊断的既往史、个人史、婚育史、月经史'],
        # If necessary, relevant past medical history, personal history, marital and reproductive history, and menstrual history that may aid in diagnosis or differential diagnosis can be appropriately added.
        'diagnosis_prompts': ['若未提供诊断信息，则不书写诊断结果部分',
                              ],
        # If no diagnostic information is provided, do not write the diagnosis section.
    }
...
input_templates_4 = {
    ...
    'requirements': {
        ...
        # After performing synonym conversion on structured information, rewrite it in natural language
        # Generated content should only involve medical history information; it is prohibited to generate content related to TCM etiology, TCM pathogenesis, syndrome differentiation analysis, four diagnostic methods analysis, syndrome differentiation results, diagnosis and treatment plans, prescriptions, formula composition, decoction methods, or precautions.
        # Do not change the original meaning of the information.
        # Avoid unnecessary repetition in the text.
        # Use medical standard language in writing, with clear logic.
        'style_prompts': [
            ...
            # Translate the structured information into a medical case study in the style and format of <space_0>, but avoid using any terminology related to traditional Chinese medicine theory, and do not specify the style being imitated.
            # He Puren, Jiao Shude, Jin Rui, Li Jinyong, Li Ke, Li Keshao, Liu Duzhou, Liu Bichen, Lu Zhizheng, Lü Bingkui, Ren Yingqiu, Qiu Peiran, Shang Tianyu, Shi Qi, Shi Xuemin, Wang Yongyan, Wang Qi, Yan Dexin, Yang Jiasan, Yin Huihe, Yue Meizhong, Zhang Qi, Zhao Bingnan, Zhao Shaoqin, Zhou Zhongying, Zhu Liangchun.
            '不必保留原结构化信息的格式'],
        'chief_complaint_prompts': ['确保主诉精简规范，主诉症状或体征合计不超过3个',
                                    '主诉不超过20个字',
                                    '若未给定主诉，则从临床表现中选择最急需解决的症状或体征作为主诉，主诉未必是前几个临床表现',
        ...
        # Ensure the chief complaint is concise and standardized, with no more than 3 symptoms or signs in total.
        # The chief complaint should not exceed 20 characters.
        # If no chief complaint is provided, select the most urgent symptom or sign from the clinical manifestations as the chief complaint; it does not necessarily have to be the first few listed.
        # If clinical manifestations involve multisystem diseases, choose a single-system disease as the primary diagnosis, which should correspond to the chief complaint.
        # Although traditional Chinese medicine information often involves symptoms and signs from multiple systems, the chief complaint should not correspond to multiple unrelated systems or Western medical diagnoses. Instead, it should correspond to a single system or a single Western medical diagnosis; other information need not be reflected in the chief complaint.

        'sym_phy_prompts': [
            '结构化信息中临床表现为乱序，需以主诉为核心调整书写顺序，主次分明，相关的临床表现相邻',
            '参考病史信息和对应处方添加指定的额外信息，但添加的额外信息不影响处方结果',
            # In structured information, the clinical manifestations are presented out of order; the writing sequence should be adjusted with the chief complaint as the core, ensuring clear prioritization and adjacent placement of related clinical manifestations.
            # Add specified additional information with reference to medical history and corresponding prescriptions, but the added information should not affect the prescription outcome.
            [
                '适当添加与当前辨证论治无关的症状体征信息，但不影响最终辨证结果，不得额外增加支持当前辨证论治结果的其他症状体征',
                ''],
            # When translating, appropriately add information about symptoms and signs unrelated to the current pattern identification and treatment, but ensure it does not affect the final diagnostic outcome. Do not add any additional symptoms or signs that would further support the current pattern identification and treatment result.
            '按病例书写习惯在刻下症处添加适当的阴性信息',
            '若未指定一般情况、进食、睡眠、大便、小便、查体，需按病例书写习惯补充相关阴性信息',
            '若个别临床表现与整体明显不符，可删去',
            '若舌象或脉象信息存在矛盾，需考虑是不同部位存在不同的舌象脉象的可能'],
            # "Add appropriate negative findings in the section for current symptoms according to medical record writing conventions."
            # "If general condition, diet, sleep, bowel movements, urination, or physical examination are not specified, relevant negative findings should be supplemented as per medical record writing conventions."
            # "If individual clinical manifestations are significantly inconsistent with the overall presentation, they may be omitted."
            # "If there are contradictions in tongue or pulse manifestations, the possibility of different tongue and pulse characteristics in different body regions should be considered."
        'pre_treat_prompts': ['可适当添加既往治疗的中西药物', ],
        # Appropriate addition of previous Chinese and Western medications used in treatment may be considered.
        'aux_exam_prompts': ['可生成额外的支持诊断或鉴别诊断的辅助检查内容', ],
        # Can generate additional auxiliary examination content to support diagnosis or differential diagnosis.
        'past_med_his_prompts': ['若结构化信息中未指定既往史，需按病例书写习惯补充相关阴性信息'],
        # If the structured information does not specify the past medical history, relevant negative information should be supplemented according to medical record writing conventions.
        'explanation_prompts': ['禁止添加关于本病例文本如何被编写的“元说明”和自我评价']
        # Do not add any "meta-explanations" or self-evaluations regarding how this case text was written.
    }
...
input_templates_5 = {
    ...
    'requirements': {
        ...
        # Generated content should only involve medical history information; it is prohibited to generate content related to TCM etiology, TCM pathogenesis, syndrome differentiation analysis, four diagnostic methods analysis, syndrome differentiation results, diagnosis and treatment plans, prescriptions, formula composition, decoction methods, or precautions.
        # Do not change the original meaning of the information.
        # Avoid unnecessary repetition in the text and avoid contradictions between preceding and following content.
        # Use medical standard language in writing, with clear logic.
        'style_prompts': [
            {
                'prompt': '将结构化信息改写为中医医案，模仿<space_0>的语言风格，但禁止使用中医理论相关词汇、严禁保留原结构化信息的格式，不需按照主诉、现病史、既往史等的顺序书写',
                # Translate the structured information into a traditional Chinese medical case record, imitating the language style of <space_0>, but strictly prohibit​ the use of terminology related to traditional Chinese medicine theory, strictly prohibit​ retaining the original structured information format, and do not need to follow the order of chief complaint, present illness, past history, etc.
                'space': [[[
                          ...
                          # Chen Xiuyuan, Cheng Guopeng, Fu Qingzhu, Gao Bingjun, Gao Shizong, He Mengyao, Huang Yuanyu, Ke Qin,
                          # Lei Feng, Li Yongcui, Lin Peiqin, Lu Maoxiu, Shen Jin'ao, Tang Zonghai, Wang Ang, Wang Qingren,
                          # Wu Jutong, Wu Qian, Xu Lingtai, Ye Tianshi, You Zaijing, Yu Jiayan, Zhang Lu, Zhang Zhicong, Zhao Xuemin,
                          # Zhou Xuehai, Cao Bingzhang, Chen Cunren, Chen Wujiu, Cheng Dan'an, Ding Ganren, Fei Boxiong, He Lianchen, Huang Zhuzhai,
                          # Yun Tieqiao, Lu Yuanlei, Qin Bowei, Ran Xuefeng, Shi Jinmo, Shi Yiren, Tan Cizhong, Tang Rongchuan,
                          # Wang Mengying, Wu Ruifu, Xie Guan, Yang Zemin, Zhang Shanlei, Zhang Xichun, Zhang Cigong,
                          # Yoshimasu Tōdō, Tamba Yasuyori, Manase Dōsan, Gotō Konzan, Kagawa Shūan, Yamawaki Tōyō,
                          # Motohiro Taki, Motoosa Taki, Asada Sōhaku, Ōtsuka Keisetsu, Tatsuno Kazuo, Yasujiro Shashi, Morimoto Dōhaku,
                          # Tōhō Junkyū, Wada Masatsura, Nakagami Kinkei, Oda Yōdō, Nagatomi Dokushōan, Genzui Hara, Honma Sōken
                          ]}, ],
        'sym_phy_prompts': [
            '若未给定主诉，则从临床表现中选择最急需解决的症状或体征作为主诉，主诉未必是前几个临床表现',
            '结构化信息中临床表现为乱序，需以主诉为核心调整书写顺序，主次分明，相关的临床表现相邻',
            '参考病史信息和对应处方添加指定的额外信息，但添加的额外信息不影响处方结果',
            # If the chief complaint is not provided, select the symptom or sign that most urgently needs to be addressed from the clinical manifestations as the chief complaint. The chief complaint does not necessarily have to be among the first few clinical manifestations listed.
            # The clinical manifestations in the structured information are in random order; the writing order should be adjusted with the chief complaint as the core, making the presentation clear and logical, with related clinical manifestations placed adjacent to each other.
            # Add specified additional information by referring to the medical history and corresponding prescriptions, but ensure that the added information does not affect the prescription outcome.
            [
                '适当添加与当前辨证论治无关的症状体征信息，但不影响最终辨证结果，不得额外增加支持当前辨证论治结果的其他症状体征',
                ''],
            # Appropriately add information about symptoms and signs unrelated to the current syndrome differentiation and treatment, but without affecting the final diagnosis. Do not add any additional symptoms or signs that would support the current diagnosis.
            '按病例书写习惯添加适当的阴性信息',
            '若个别临床表现与整体明显不符，可删去',
            '若舌象或脉象信息存在矛盾，需考虑是不同部位存在不同的舌象脉象的可能'],
        # Add appropriate negative findings according to clinical documentation practices.
        # If individual clinical manifestations are significantly inconsistent with the overall presentation, they may be omitted.
        # If there are contradictions in tongue or pulse findings, consider the possibility of different tongue and pulse characteristics in different body regions.
        'pre_treat_prompts': ['可适当添加既往治疗', ],
        # Appropriately add previous treatments.
        'explanation_prompts': ['禁止添加关于本病例文本如何被编写的“元说明”和自我评价']}
        # Do not add any "meta-explanations" or self-evaluations regarding how this case text was written.
}
...
output_template = {
    'head': '医案及处方信息如下：',
    # The medical case and prescription information are as follows:
    'task': {
        'task_prompts': ['根据给定的中医医案及处方信息，从理、法、方、药多个角度分析医案，并进行中医鉴别诊断，回顾相关中医知识，最后规范写出带有剂量的处方、煎服法及注意事项',]
    # Based on the given traditional Chinese medicine (TCM) medical case and prescription information, analyze the case from multiple perspectives including etiology, pathogenesis, treatment principles, formulas, and herbs. Perform TCM differential diagnosis, review relevant TCM knowledge, and finally write a standardized prescription with specified dosages, decoction and administration methods, and precautions.
    },
    'requirements': {
        'basic_requirement_prompts': [
            '不改变原信息含义',
        ...
        # "Without altering the original meaning of the information,"
        # "Written in the language style and format of a modern TCM practitioner."
        'diagnosis_prompts': ['除非提及中西医诊断，否则不书写任何诊断结果'],
        # Unless Western or Chinese medicine diagnosis is mentioned, do not write any diagnostic results.
        'mdf_exp': [
            '分析方药时，若结构化信息提供了加减法注释，将加减法注释改写精简且尽可能包含原信息，体现药证，可适当根据加减法注释推理加减法部分的中医病机'],
        # When analyzing formulas and herbs, if structured information provides modification notes (additions/subtractions), rewrite and condense these notes to be as concise as possible while retaining the original information. Ensure the modifications reflect the pattern-herb correspondence, and appropriately infer the underlying disease mechanisms in traditional Chinese medicine based on the modification notes.
        'prescriptions': ['书写处方时不加减方药，不改变原方药、不改变方药剂量、不改变方药剂量单位、不改变煎服法'],
        # When writing a prescription, do not add or subtract any ingredients, do not alter the original formula, do not change the dosage of the ingredients, do not modify the units of measurement for the dosage, and do not change the decoction and administration methods.
        'notes': ['可适当补充必要的注意事项'],
        # You can add necessary precautions as appropriate.
        'explanation_prompts': ['禁止添加关于本病例文本如何被编写的“元说明”和自我评价',
                                '生成内容中禁止提到任何上述文本生成要求的内容']
        # Do not add any "meta-explanations" or self-evaluations regarding how this case text was written.
        # Generated output must not mention any of the above text generation requirements.
    }
}
